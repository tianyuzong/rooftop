...
4. 只读与内部审计写入自动继续；三档策略通过全部确定性风险门禁后自动激活，配置晋级和回滚等其他高影响写入进入 `WAITING_APPROVAL`。
...
该工作流先冻结投资授权书，再为激进、平衡、保守三档生成参数候选。回测以 126 个交易日为一期：第一期冻结使用基线参数；以后每期开始前只根据已经结束的半年结果重选参数，本期结果只能参与下一期更新。最后一期在选参完成后只对入选参数和基线运行一次独立留出检查。三档全部通过滚动验证、最终留出风险与非退化门禁时，由 Rooftop 系统身份立即原子激活；未通过则保存为持久重试任务，在下一个完成数据更新的交易日用下一批白名单参数继续评测。相同最终留出快照不会被反复用于调参，结果始终包含 `order_execution=false`。
...
先复核实验是否成功且三档策略均通过半年递推样本外验证、最终留出风险门禁和基线非退化门禁，通过后自动激活，不再等待人工批准。目标收益未命中不会单独禁止激活，因为它是软目标；风险或非退化门禁未通过一定禁止激活，并进入跨交易日持续优化队列。
...
在线模型的日常更新与首次整窗晋级分开：从当前活动系数继续学习晚于 `training_end` 的新增完整交易日，以冻结的当前活动模型在同一区间作对照，通过累计样本量、非退化和最大回撤门禁即可发布日常检查点，不额外要求每天显著改善。增量评测披露实际天数和样本数，不称为半年验证。首次整窗训练、深度模型、组合和源码晋级仍要求各自的改善及独立留出门禁。

盘后在在线模型和组合刷新之前增加基本面阶段，保存新财报与当日估值快照，并用同一收盘日期重新评测三档策略。Skill 文本是稳定控制面，不由每日数据重写；运行制品才进行版本切换。股票池必须逐股检查缺失和陈旧数据；任何必需阶段或组合失败时整轮记为 `PARTIAL` 并保留重试资格，组合重试只补跑失败授权书。
...
配置候选晋级和版本回滚在执行最终写入前进入 `WAITING_APPROVAL`；三档策略激活已改为确定性门禁通过后自动执行。其他操作的批准接口：
...
- `CONSEQUENTIAL_WRITE`：切换当前配置或回滚配置版本，必须人工批准。通过全部确定性门禁后的三档研究策略激活属于 `INTERNAL_WRITE`，由系统自动记录激活身份。
