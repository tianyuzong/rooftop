...
需求及精选研究证据会发送至本机登录的 Codex 账户，访问令牌不会发送给模型。登录框支持输入令牌或选择本机令牌文件。
...
例如：假设本金 10 万元、某策略单只仓位上限 16.67%，单只最多分配约 16,670 元；若示例股参考价为 400 元，一手 100 股需要 40,000 元，就会计算为 0 股。页面会解释未分配原因，而不是把 0 股展示成已形成的投入方案。该示例仅说明计算规则。
...
前端回归覆盖 0 股分配解释、缺少运行编号时的详情展示、结果定位和读取失败恢复。GitHub Actions 当前提供手动 `workflow_dispatch` 测试入口。
