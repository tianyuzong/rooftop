...
| 按本金和风险条件筛选股票 | 股票推荐 | 筛选结论、可分配股数、现金余量及未形成方案的原因 |
| 比较几家公司 | 股票对比 | 2–8 只股票的资料、评分与风险比较 |
| 收到昨天大盘或关注公司的简报 | 信号中心 | 可选邮件内容、发送时间、预览、发送记录和收件确认 |
| 查看行情或管理自己的股票 | 市场总览 | 行情图表；只有自行确认导入后才显示真实持仓 |

首次使用先完成[安装](#6-安装)，再运行 `./scripts/start_stock_compare.ps1 -View signals`，打开脚本输出的 `url`。无需先导入持仓，也无需先配置邮箱；邮件配置完成后才会外发。
...
填写条件后点击“按这些条件查看推荐”，页面会直接定位到“本次结论”，展示已读取的结果；若数据不足，原因也会显示在同一区域。结果包括模型筛选、资金分配和历史验证，三者应分别查看。
...
| 暂无可投入组合 | 可能已有筛选通过的公司，但分配金额不足一手；页面列出单只仓位上限和一手所需金额 |
| 数据不足或读取失败 | 显示缺失数据或错误原因，不以成功提示代替结果 |

“最多持有”只限制股票数量。金额按综合评分、波动和整手约束分别计算，单只仓位上限独立取决于风险档位（激进 60%、中立 45%、保守 35%），不会由最大持有数量推导。允许少于数量上限，也允许各只金额不同；余款保留为现金。即时推荐与新回测共用这套分配方法，旧版组合重新分配后作为待盘后验证的快照显示。

例如：本金 10 万元、激进档、最多持有 6 只，若只有一只股票通过筛选，其单只上限仍为 6 万元。按示例参考价 412.22 元，取整后可分配 100 股、41,222 元，余款 58,778 元。若一手金额确实超过独立风控上限，页面仍会解释无法分配的原因。该示例仅说明计算规则。
...
- 服务启动立即同步，此后每 60 秒核对通达信报价；休市继续核对收盘数据，价格可能不变。查看中的分钟图自动补齐，切回页面立即更新。
- 日线：默认每 6 小时检查一次。
- 历史：默认每 24 小时检查一次。
- 研报/公告：默认每小时检查当天增量。
- 持续学习：交易日 15:01 起自动启动盘后数据检查、策略评测与推荐更新；数据未齐或失败时重试，法定休市日跳过。
- 行情状态栏：每秒显示当前时间，每 5 秒读取真实同步状态；行情和所选 K 线每 60 秒刷新，最近同步时间仅在整批成功后推进。
...
前端回归覆盖 0 股分配解释、缺少运行编号时的详情展示、结果定位、休市缓存读取、后台状态隔离、失败保留结果和旧请求回写防护。GitHub Actions 当前提供手动 `workflow_dispatch` 测试入口。
...
继续使用原来的数据湖、Python 环境和远程访问参数。数据库结构会在启动时迁移；仓库不会提供用户的持仓、收件邮箱、数据库、授权码或模型权重。源码目录也不等于 Codex 已安装的插件缓存，使用缓存安装的用户还需按原安装方式更新插件。
...
正式版本说明模型记录已经发布，不保证当前本金和仓位约束可以买入整手。页面会直接显示本次结论，并解释没有分配的公司、一手所需金额和单只仓位上限。
...
即时研究排名覆盖所选板块中历史数据齐备的候选，不再按成交额截取前 30 只；盘后完整回测仍保留独立的计算预算。研究名单优先展示本金可覆盖最小买入单位的股票，走势或模型概率未过门槛仅作为不足说明。正式仓位与试探方案单独展示，研究排名不代表通过组合风控。最多持仓数仅为上限，候选不足上限时仍可排名。

### 筛选宽松度

推荐页提供 0–100 滑块，界面初始值 50，旧 API 请求未传参数时保持 0。设宽松度比例为 r（滑块值 / 100），上涨概率门槛下降 15r 个百分点，价格及短期均线可低于长期均线 10r%，动量允许下跌 20r%，基本面最低分下降 0.30r。财务覆盖要求、历史数据要求、本金、最大回撤、止损和仓位上限不受影响。调整后提交才生效；请求身份包含该参数，盘后回测和即时筛选使用同一计算方法。筛选通过数量并不等于正式组合已验证。

### 历史与预测衔接图

个股图左侧显示已有历史收盘价，右侧显示经过期限检验的中位数连接线和区间色带，分界处标明数据截止日。两侧共用价格纵轴，历史与未来的时间横轴分段缩放；默认展示最近 30–90 个交易日中的适当历史窗口，更长历史与较宽区间可展开。预测连线与色带边缘仅连接已有的已检验期限节点，不生成虚构的每日价格或每日区间。原始分位数、覆盖率与中位变化金额保持不变。
