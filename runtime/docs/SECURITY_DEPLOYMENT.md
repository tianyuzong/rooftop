...
Rooftop 默认只监听 `127.0.0.1`。外部试用应采用“反向代理 TLS + API 令牌 + 防火墙来源白名单”，不要把开发服务直接暴露到公网。

## 最低要求

1. 使用随机高强度令牌文件，并通过 `-TokenFile` 启动。非回环监听没有令牌会直接拒绝启动。
2. 使用 Caddy、Nginx、IIS 或同类反向代理终止 HTTPS；Rooftop 后端仍监听内网地址。
...
本机通过 `http://127.0.0.1:8765`、`localhost` 或 `[::1]` 直接打开页面时可自动连接，无需输入远程令牌。服务端同时校验实际回环连接、Host 和端口、同源信息及页面请求标记；远程与转发请求继续校验令牌。配置白名单时须包含要使用的本机 Host 与 Origin。反向代理必须保留 `Forwarded` 或 `X-Forwarded-*` 等转发标头，不能将外部请求伪装成本机直连。此方式信任本机运行账户，不提供本机用户之间的隔离。
