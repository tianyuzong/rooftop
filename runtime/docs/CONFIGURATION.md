...
| `-View` | `auto` | `auto/signals/compare/harness` | `auto` 有股票时进入对比，否则进入信号中心 |
...
| 变量 | 默认 | 必填条件 | 说明 |
|---|---|---:|---|
| `ARGUS_SMTP_HOST` | 空 | 外发时是 | SMTP SSL 主机 |
| `ARGUS_SMTP_PORT` | `465` | 否 | SMTP SSL 端口 |
...
SMTP 主机、用户和密码必须同时存在才显示 configured。即使配置完整，未启用发送时仍为 dry-run。
...
5. 保持 `ARGUS_EMAIL_SEND_ENABLED=0`，除非已完成邮件合规和退订流程。
