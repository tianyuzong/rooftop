...
async function request(url, options = {}, retried = false) {
  ...
  if (response.status === 401 && !retried) {
    const supplied = window.prompt('请输入 Argus 远程访问令牌');
    if (supplied) {
...
function bindPortfolioImport() {
  ...
  $('#portfolioTemplateDownload').onclick = () => {
    ...
    link.href=url;link.download='Argus-持仓导入模板.csv';link.click();URL.revokeObjectURL(url);
  };
}
...
function quantTimeframeForecastHtml(item){
  ...
  return `<article class="quant-forecast-card ${actionClass}"><header><div><strong>${safe(item.name)} <small>${safe(item.symbol)}</small></strong><span>${safe(item.action_label||summary.action_label||'继续观察')} · ${safe(validationLabel)} · ${safe(basisLabel)}</span></div>${sellHtml}</header>${forecastPointMetricsHtml(forecast)}${stockForecastCurveSvg(forecast)}<footer><span>绿色实线：最近90根日K</span><span>${hasBaseline?'黄色点线：历史基准中位情景':'黄色虚线：校准中位数'}</span><span>${hasBaseline?'色带：历史重采样概率范围':'深浅色带：样本外检验后的概率区间'}</span></footer></article>`;
}

function stockForecastCurveSvg(forecast){
  const history=(forecast.history_curve||[]).filter(item=>Number.isFinite(Number(item.price))),future=(forecast.forecast_curve||[]).filter(item=>Number.isFinite(Number(item.trading_day))&&['p10','p25','p50','p75','p90'].every(key=>Number.isFinite(Number(item[key]))));
  if(history.length<2)return '<div class="empty-state">现有日K不足，无法进行滚动样本外校准</div>';
  const hasForecast=future.length>1,hasBaseline=future.some(item=>item.basis==='HISTORICAL_BASELINE_REFERENCE'),maxForecastDay=hasForecast?Math.max(...future.map(item=>Number(item.trading_day))):0,w=820,h=260,left=64,right=24,top=28,bottom=48,plotRight=w-right,split=hasForecast?left+(plotRight-left)*.43:plotRight;
  ...
  const base=Number(history.at(-1).price),decimals=base<10?3:2,phaseLabel=hasBaseline?'历史基准概率情景':'样本外校准区间',forecastSvg=hasForecast?`<polygon class="forecast-band-wide" points="${bandPoints('p90','p10')}"/><polygon class="forecast-band-likely" points="${bandPoints('p75','p25')}"/><line class="forecast-start" x1="${split}" y1="${top}" x2="${split}" y2="${h-bottom}"/><line class="forecast-base" x1="${split}" y1="${y(base).toFixed(1)}" x2="${plotRight}" y2="${y(base).toFixed(1)}"/><polyline class="forecast-median ${hasBaseline?'baseline-reference':''}" points="${futurePoints('p50')}"/><text class="phase-label" x="${split+8}" y="${top+13}">${phaseLabel}</text>`:'';
  const horizonNote=forecast.horizon_status==='FULL'?`已覆盖请求的 ${Number(forecast.requested_horizon_trading_days||maxForecastDay)} 个交易日`:(forecast.horizon_reason||`当前情景展示到 ${maxForecastDay} 个交易日`),title=hasBaseline?'历史价格与历史基准概率情景':'历史价格与样本外校准概率路径',description=hasBaseline?'绿色实线为最近日K收盘价，黄色点线与阴影为历史收益重采样基准情景；它没有通过预测优势门禁。':'绿色实线为最近日K收盘价，黄色虚线为经滚动样本外检验的中位数，阴影为校准概率区间。';
  return `<div class="quant-chart-wrap stock-forecast-chart"><svg class="quant-equity-chart" viewBox="0 0 ${w} ${h}" role="img" aria-label="${safe(String(forecast.symbol||''))}${safe(title)}"><title>${safe(title)}</title><desc>${safe(description)}</desc><line class="chart-grid" x1="${left}" y1="${top}" x2="${plotRight}" y2="${top}"/><line class="chart-grid" x1="${left}" y1="${h-bottom}" x2="${plotRight}" y2="${h-bottom}"/>${forecastSvg}<polyline class="history-line" points="${historyPoints}"/><text x="${left}" y="${top+13}">${money(max,decimals)}</text><text x="${left}" y="${h-bottom-6}">${money(min,decimals)}</text><text x="${left}" y="${h-12}">${safe(history[0].date)}</text><text x="${split}" y="${h-12}" text-anchor="middle">${safe(history.at(-1).date)}</text>${hasForecast?`<text x="${plotRight}" y="${h-12}" text-anchor="end">${hasBaseline?'基准情景':'已验证'} ${maxForecastDay} 个交易日</text>`:''}</svg><div class="quant-chart-legend"><span class="legend-history">历史收盘走线</span><span class="legend-median">${hasBaseline?'基准中位情景':'校准中位数'}</span><span class="legend-likely">核心范围 P25—P75</span><span class="legend-wide">${hasBaseline?'历史范围 P10—P90':'已检验范围 P10—P90'}</span></div><small class="quant-chart-note">${hasForecast?safe(horizonNote):safe(forecast.horizon_reason||'没有足够历史数据生成情景。')} ${hasBaseline?'历史基准情景未证明有预测优势；':''}概率路径不是确定价格或收益承诺。</small></div>`;
}
...
function quantRuleSnapshotHtml(quant,rec){
  ...
  return `<section class="quant-result rule-snapshot-result"><div class="quant-result-head"><div><span class="status-tag">${safe(rec.profile_label||rec.profile||'中立')}</span><strong>即时多因子与K线预测</strong><small>数据截至 ${safe(rec.data_asof||'—')} · 使用本地缓存</small></div><b>${safe(quantVersionLabel(version.status))}</b></div>
    <div class="quant-decision-callout rule_snapshot"><span>按你的本金、期限与风险条件即时计算</span><strong>${actionSummary?safe(actionSummary):'当前不建议为候选股票分配资金，全部保留现金'}</strong><small>这是研究分配建议，不是你的真实持仓，也不会自动下单；实际成交前需人工复核价格与风险。</small></div>
...
function quantDecisionInput(){return {...state.quantDraft,take_profit_mode:'trailing'};}
...
function quantDecisionPanelHtml(_detail,learning,_quantVersions,sectorCache){
  ...
  return `<section class="panel decision-entry"><div class="panel-head"><div><p class="eyebrow">投资条件</p><h2>填写条件，查看股票推荐</h2></div><div class="daily-decision-state"><span>${dataState}</span><strong>${safe(freshness)}</strong><small>${version?`结果编号 ${safe(version.version_key)}`:'还没有可用结果'}</small></div></div>
    ...
    <div class="decision-form">
      ...
      <label>止损（%）<input data-quant-draft="stop_loss_pct" id="quickStopLoss" type="number" min="1" max="80" value="${Number(d.stop_loss_pct)}"></label>
    ...
    <details class="decision-options professional-disclosure"><summary>专业参数与更多选项</summary><div>
      ...
      <label>盈利后回落多少时止盈（%）<input data-quant-draft="trailing_stop_pct" id="quickTrailingStop" type="number" min="1" max="80" value="${Number(d.trailing_stop_pct)}"></label>
...
function renderSignalCenter(data, models){
  ...
  $('#subscriptionList').innerHTML=subscriptions.length?subscriptions.map(item=>`<div class="subscription-row">
    <div><strong>${safe(item.name)}</strong><span>${safe(item.target||subscriptionData.smtp?.default_target||'服务器默认邮箱')} · ${(item.event_kinds||[]).map(signalActionLabel).join(' / ')}</span></div>
