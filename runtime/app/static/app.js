const state = { dashboard: null, selected: null, chart: null, period: '1d', chartWindow: null,
  ...
  harnessRunKey:null, harnessWorkflow:'quant_portfolio', quantDecision:null, quantDraft:{name:'白酒新能源半导体航天2%组合',capital:100000,
    ...
    max_positions:2,risk_profile:'balanced',screening_relaxation:50,max_iterations:10,backtest_window_years:3,
...
function askAccessToken() {
  ...
  tokenDialogPromise = new Promise(resolve => {
    const dialog = document.createElement('dialog');
    dialog.className = 'agent-token-dialog';
    dialog.innerHTML = '<form method="dialog"><h2>连接 Rooftop</h2><p>请输入 Rooftop 远程访问令牌，或选择本机令牌文件。</p><label>访问令牌<input name="token" type="password" autocomplete="off" required></label><label class="agent-token-file">选择令牌文件<input name="tokenFile" type="file" accept=".token,.txt,text/plain"></label><p class="agent-muted">令牌仅用于当前浏览器会话，不会发送给 Codex。</p><div class="agent-submit-row"><button type="button" class="secondary-button" data-cancel>取消</button><button type="submit" class="primary-button">连接</button></div></form>';
    document.body.append(dialog);
    const input = dialog.querySelector('[name="token"]');
    dialog.querySelector('[name="tokenFile"]').onchange = async event => {
      const file = event.target.files?.[0];
      if (file && file.size <= 4096) input.value = (await file.text()).trim();
    };
    const finish = value => {dialog.close();dialog.remove();tokenDialogPromise=null;resolve(value);};
    dialog.querySelector('form').onsubmit = event => {event.preventDefault();finish(input.value.trim());};
...
async function request(url, options = {}, retried = false) {
  ...
  if (response.status === 401 && !retried) {
    const supplied = await askAccessToken();
    if (supplied) {
...
function renderSummary(portfolio, evidenceCoverage = null) {
  ...
  $('#portfolioSummary').hidden = !hasPositions;
  $('#portfolioSection').hidden = !hasPositions;
  $('#portfolioEmptyNote').hidden = hasPositions;
...
async function refreshRealtimeOverview(){
  ...
  try{
    const dashboard=normalizeDashboard(await request('/api/dashboard'));state.dashboard=dashboard;renderMarketSyncState(dashboard.meta);if(document.querySelector('#overviewView.active')){const assets=dashboard.analysis_assets.length?dashboard.analysis_assets:[...dashboard.portfolio.positions,...dashboard.markets.filter(m=>!dashboard.portfolio.positions.some(p=>p.symbol===m.symbol))];state.overviewAssets=Object.fromEntries(assets.map(item=>[item.symbol,{symbol:item.symbol,name:item.name}]));$('#assetSuggestions').innerHTML=assets.map(item=>`<option value="${safe(item.name)} ${safe(item.symbol)}"></option>`).join('');if(state.selected?.symbol){const response=normalizeChartPayload(await request(`/api/assets/${encodeURIComponent(state.selected.symbol)}/chart?period=${encodeURIComponent(state.period)}`),state.period),wasAtEnd=!state.chartWindow||state.chartWindow.end>=(state.chart?.series?.length||0);state.chart=response;if(wasAtEnd&&state.chartWindow){const span=state.chartWindow.end-state.chartWindow.start;state.chartWindow.end=response.series.length;state.chartWindow.start=Math.max(0,response.series.length-span);}renderChart(response);const livePrice=response.quote?.price??response.series.at(-1)?.close,change=response.quote?.change_pct??response.series.at(-1)?.change_pct;$('#assetPrice').textContent=Number(livePrice)>0?`¥ ${money(livePrice,3)}`:'待行情';$('#assetPnl').textContent=change==null?'—':pct(change);$('#assetPnl').style.color=chartTrendColor(change||0);$('#asOf').textContent=response.quote?.observed_at||response.series.at(-1)?.time||'—';}}
  }catch(error){$('#dataMode').textContent='⚠ 无法读取同步状态，正在重试';$('#dataMode').classList.add('negative');console.warn('realtime overview refresh failed',error);}finally{state.realtimeBusy=false;}
}

async function refreshMarketSyncStatus(){
  if(document.hidden||state.syncStatusBusy)return;
  state.syncStatusBusy=true;
  try{const meta=await request('/api/market-sync');state.syncStatusError=false;if(state.dashboard)state.dashboard.meta={...state.dashboard.meta,...meta};renderMarketSyncState(meta);}
  catch(error){state.syncStatusError=true;$('#dataMode').textContent='⚠ 无法读取同步状态，正在重试';}
  finally{state.syncStatusBusy=false;}
}

function renderMarketSyncState(meta){
  if(!meta)return;
  const health=meta.quote_sync||{},last=health.last_success_at;
  const sync=last?` · 最近整批同步 ${localTimestamp(last)}`:(meta.last_sync_at?` · 本地缓存 ${localTimestamp(meta.last_sync_at)}`:'');
  const bad=['FAILED','PARTIAL','STALE','WAITING'].includes(health.status)||!health.status;
  let label='○ 自动同步未启用';
  if(meta.live_refresh_enabled){
    if(bad)label=health.status==='PARTIAL'?`⚠ 通达信部分更新（${health.received_count||0}/${health.requested_count||0}），每分钟重试`:'⚠ 通达信同步异常，每分钟重试';
    else label=meta.market_session_open?'● 通达信盘中每分钟同步':'● 通达信每分钟核对 · 已收盘/休市，价格可能不变';
    if(!meta.market_session_open&&bad)label='⚠ 通达信同步异常，休市仍每分钟重试';
  }
  $('#dataMode').textContent=label+sync+` · 当前时间 ${localTimestamp(new Date().toISOString())} · 交易日15:01起更新策略`;
  $('#dataMode').classList.toggle('negative',Boolean(meta.live_refresh_enabled&&bad));
}
...
function renderLogicView(methodology,decision){
  ...
  <section class="logic-workbench"><div class="logic-section-head"><div><p class="eyebrow">你的推荐设置</p><h2><span class="beginner-only">选择你更看重什么</span><span class="professional-only">调整后保存，直接影响股票推荐</span></h2></div><div class="logic-presets"><button data-logic-preset="aggressive" class="${Object.entries(logicPreferencePresets('aggressive')).every(([k,v])=>Number(preset[k])===v)?'active':''}">偏走势</button><button data-logic-preset="balanced" class="${Object.entries(logicPreferencePresets('balanced')).every(([k,v])=>Number(preset[k])===v)?'active':''}">均衡</button><button data-logic-preset="conservative" class="${Object.entries(logicPreferencePresets('conservative')).every(([k,v])=>Number(preset[k])===v)?'active':''}">偏财务与稳定</button></div></div><p class="logic-note strong beginner-only">这组选项改变股票评分权重；激进 / 中立 / 保守另外控制组合风险限制。选择后点击保存生效。没有全部过关的股票时，推荐结果仍会列出相对优选。</p><p class="logic-note strong professional-only">滑动时先预览排序；点击保存后，后端会用同一组设置重算当前推荐，并在下一个交易日盘后完成滚动回测。未通过样本外检查的设置不会覆盖上一正式版本。</p><div class="logic-controls professional-only"><label>细分选股方法<select id="logicStrategyStyle"><option value="auto" ${state.quantDraft.strategy_style==='auto'?'selected':''}>自动比较全部方法</option>${styleOptions}</select></label><label>每天回看多长历史<select id="logicBacktestWindow"><option value="1" ${Number(state.quantDraft.backtest_window_years)===1?'selected':''}>近1年（另加指标预热数据）</option><option value="3" ${Number(state.quantDraft.backtest_window_years)===3?'selected':''}>近3年（默认，约756个交易日）</option><option value="5" ${Number(state.quantDraft.backtest_window_years)===5?'selected':''}>近5年</option></select></label></div><div class="logic-simulator professional-only"><div class="logic-sliders">${sliders}<small id="logicWeightTotal"></small><p>综合分 = 五项分数 × 你的权重。成交活跃度来自成交额，不等于“主力资金”。</p></div><div class="table-wrap"><table class="logic-ranking"><thead><tr><th>#</th><th>股票</th><th>预览综合分</th><th>走势</th><th>财务</th><th>概率</th><th>活跃</th><th>稳定</th><th>当前状态</th></tr></thead><tbody id="logicSimulationBody"></tbody></table></div></div><div class="logic-workbench-actions"><button id="logicApplyPreferences" class="primary-button">保存并用于股票推荐</button><button id="logicOpenRecommendation" class="secondary-button">查看推荐结果</button></div></section>
...
function harnessStatus(status){return({OBSERVED:'待补期望',READY:'待评测',RESOLVED:'已解决',IGNORED:'已忽略',DRAFT:'候选草案',EVALUATED:'已评测',ACTIVE:'已生效',SUCCESS:'通过',SUCCESS_WITH_WARNINGS:'完成，有警告',WARNING:'警告',FAILED:'失败',ERROR:'错误',QUEUED:'排队中',PENDING:'等待新数据',RUNNING:'运行中',RUNNING_MARKET:'正在补充行情数据',RUNNING_FUNDAMENTALS:'正在补充财务数据',COMPLETED_WITH_WARNINGS:'完成，有缺口',WAITING_APPROVAL:'等待批准',COMPLETED:'已完成',INTERRUPTED:'可恢复',CANCELLED:'已取消',PROPOSED:'待执行',RETRYING:'重试中',RETRY_SCHEDULED:'已安排重试',WAITING_FOR_NEW_DATA:'等待新交易日',PENDING_NEW_DATA:'等待新交易日',SUCCEEDED:'成功',SKIPPED:'已跳过',APPROVED:'已批准',REJECTED:'已拒绝'}[status]||status);}
...
function stockForecastCurveSvg(forecast, expanded=false){
  ...
  const history=historyAll.slice(-(expanded?90:Math.max(30,Math.min(90,maxForecastDay*2))));
  ...
  const base=Number(origin?.p50||history.at(-1).price),decimals=base<10?3:2,baseline=endpoint?.basis==='CALIBRATED_BASELINE',medianLabel=baseline?'校准历史基准中位数':'校准中位数';
  const w=900,h=340,left=64,right=105,top=56,bottom=68,plotRight=w-right,plotBottom=h-bottom;
  const split=hasForecast?Number((left+(plotRight-left)*.55).toFixed(2)):plotRight;
  const values=[...history.map(item=>Number(item.price)),...future.flatMap(item=>[Number(item[lowKey]),Number(item[highKey]),Number(item.p50)])];
  const rawMin=Math.min(...values),rawMax=Math.max(...values),padding=Math.max((rawMax-rawMin)*.12,Math.abs(base)*.005,.01),min=rawMin-padding,max=rawMax+padding;
  const y=value=>Number((top+(max-Number(value))/(max-min)*(plotBottom-top)).toFixed(2));
  const xHistory=index=>Number((left+index/(history.length-1)*(split-left)).toFixed(2));
  const xFuture=day=>Number((split+Number(day)/maxForecastDay*(plotRight-split)).toFixed(2));
  const historyPoints=history.map((item,index)=>`${xHistory(index)},${y(item.price)}`).join(' ');
  const futurePoints=key=>future.map(item=>`${xFuture(item.trading_day)},${y(item[key])}`).join(' ');
  const bandPoints=hasForecast?`${futurePoints(highKey)} ${[...future].reverse().map(item=>`${xFuture(item.trading_day)},${y(item[lowKey])}`).join(' ')}`:'';
  const ticks=[max,max-(max-min)/3,min+(max-min)/3,min].map(value=>`<line class="chart-grid" x1="${left}" x2="${plotRight}" y1="${y(value)}" y2="${y(value)}"/><text x="${left-10}" y="${y(value)+4}" text-anchor="end">${money(value,decimals)}</text>`).join('');
  const difference=hasForecast?Number(endpoint.p50)-base:0,change=base?difference/base*100:0,changeText=`${difference>=0?'+':''}${money(difference,decimals)} 元（${change>=0?'+':''}${change.toFixed(2)}%）`;
  ...
  const rangeSummary=hasForecast?`<div class="stock-forecast-points" style="grid-template-columns:repeat(auto-fit,minmax(180px,1fr))"><div><span>未来 ${maxForecastDay} 个交易日 · ${core?'核心区间':'宽区间'}</span><strong>¥${money(endpoint[lowKey],decimals)}—${money(endpoint[highKey],decimals)}</strong><small>${bandLabel}</small></div><div><span>${medianLabel}</span><strong>¥${money(endpoint.p50,decimals)}</strong><small>相对参考价 ${changeText}</small></div><div><span>历史样本外实测覆盖</span><strong>${measured==null?'暂无':`${(Number(measured)*100).toFixed(1)}%`}</strong><small>${count} 个检验样本${core?' · 核心区间之外的情况仍较常见':''}</small></div></div>`:'';
  const forecastSvg=hasForecast?`<rect class="forecast-region" x="${split}" y="${top}" width="${plotRight-split}" height="${plotBottom-top}"/><polygon class="${core?'forecast-band-likely':'forecast-band-wide'}" points="${bandPoints}"/><polyline class="forecast-envelope" points="${futurePoints(highKey)}"/><polyline class="forecast-envelope" points="${futurePoints(lowKey)}"/><line class="forecast-start" x1="${split}" x2="${split}" y1="${top-5}" y2="${plotBottom}"/><line class="forecast-base" x1="${split}" x2="${plotRight}" y1="${y(base)}" y2="${y(base)}"/><polyline class="forecast-median" points="${futurePoints('p50')}"/>${future.map(item=>`<circle class="forecast-node" data-trading-day="${Number(item.trading_day)}" cx="${xFuture(item.trading_day)}" cy="${y(item.p50)}" r="4" fill="#ffd166"><title>${Number(item.trading_day)===0?'参考收盘':`未来 ${Number(item.trading_day)} 个交易日`}：¥${money(item.p50,decimals)}</title></circle>`).join('')}<text class="forecast-end-label" x="${plotRight+10}" y="${y(endpoint.p50)+4}">¥${money(endpoint.p50,decimals)}</text><text class="forecast-phase-title" x="${(split+plotRight)/2}" y="27" text-anchor="middle">未来预测 · ${maxForecastDay} 个交易日</text><text class="forecast-phase-date" x="${(split+plotRight)/2}" y="45" text-anchor="middle">虚线：中位数 · 色带：${core?'核心':'较宽'}区间</text><text x="${split}" y="${plotBottom+23}" text-anchor="middle">历史 / 预测分界</text><text x="${split}" y="${plotBottom+43}" text-anchor="middle">${safe(history.at(-1).date)}</text><text x="${plotRight}" y="${plotBottom+43}" text-anchor="end">+${maxForecastDay} 交易日</text>`:'';
  const title=hasForecast?`历史价格与未来 ${maxForecastDay} 个交易日预测区间`:'历史收盘走势';
  const detail=!expanded&&hasForecast&&(core||checked.length>1)?`<details class="audit-details"><summary>展开历史全貌与至 ${Number(checked.at(-1).trading_day)} 个交易日的宽区间 P10—P90</summary>${stockForecastCurveSvg(forecast,true)}</details>`:'';
  return `<div class="quant-chart-wrap stock-forecast-chart" data-forecast-view="${expanded?'wide':'focus'}" data-horizon-days="${maxForecastDay}">${rangeSummary}<svg class="quant-equity-chart history-forecast-chart" data-axis="history-and-forecast" data-split-x="${split}" viewBox="0 0 ${w} ${h}" role="img" aria-label="${safe(forecast.symbol)}${title}"><title>${title}</title><desc>左边为已有历史收盘价，右边为已检验的未来期限中位数和区间。两侧共用价格纵轴，时间横轴分段缩放。节点之间的连线与色带边缘仅作展示连接，不是逐日价格或每日区间预测。</desc>${ticks}${forecastSvg}<polyline class="history-line" points="${historyPoints}"/><text class="history-phase-title" x="${(left+split)/2}" y="27" text-anchor="middle">已有走势 · ${history.length} 个交易日</text><text class="forecast-phase-date" x="${(left+split)/2}" y="45" text-anchor="middle">历史收盘价</text><text x="${left}" y="${plotBottom+43}">${safe(history[0].date)}</text></svg><div class="quant-chart-legend"><span class="legend-history">已有历史</span>${hasForecast?`<span class="legend-median">未来中位数连接线</span><span class="${core?'legend-likely':'legend-wide'}">${bandLabel}</span>`:''}</div><small class="quant-chart-note">${hasForecast?'左侧历史、右侧预测，共用价格纵轴；时间横轴分段缩放。连线与色带边缘只连接已检验节点，不代表逐日预测。':'没有通过检验的未来区间，仅展示已有历史。'}长期历史基准在专业表格中查看。</small>${detail}</div>`;
}
...
function quantAllocationState(quant){
  const rec=quant?.recommendation||{},positions=rec.positions||[],request=quant?.request||{};
  const funded=positions.filter(item=>Number(item.shares)>0&&Number(item.reference_price)>0);
  const unfunded=positions.filter(item=>!funded.includes(item));
  const hasResearch=(rec.research_recommendations||[]).some(item=>Number(item.shares)>0&&Number(item.reference_price)>0);
  return {funded,unfunded,blocked:unfunded.length>0&&!funded.length&&!hasResearch,
    lotSize:Number(request.execution?.lot_size)||100,capital:Number(request.capital)||0,
    maxPositionWeight:Number(rec.parameters?.max_position_pct)||0};
}

function quantDecisionSummary(decision){
  const allocation=quantAllocationState(decision?.result);
  if(decision?.result?.recommendation?.relative_recommendations?.length)return decision.summary;
  if(allocation.blocked){
    const names=allocation.unfunded.map(item=>item.name||item.symbol).join('、');
    return `当前没有可投入的研究组合：${names}通过模型筛选，但当前分配不足一手；本金 ¥${money(allocation.capital,0)} 暂未分配。`;
  }
  return decision?.summary||'推荐结果已生成';
}

function quantResultHtml(result){
  const quant=result.quant_portfolio||{},rec=result.recommendation||quant.recommendation||{},exp=rec.expectation||{},metrics=rec.holdout_metrics||{},funding=quantAllocationState({...quant,recommendation:rec}),positions=funding.funded,rules=rec.rules||{},requestData=quant.request||{},maxPositions=Math.max(1,Number(requestData.max_positions||8)),trials=(rec.research_recommendations||[]).slice(0,maxPositions),inference=quant.inference||result.inference||{},ruleSnapshot=inference.kind==='CACHED_RULE_SNAPSHOT',snapshot=inference.kind==='LATEST_TRADING_DAY_SNAPSHOT';
  ...
  const formalSymbols=new Set(positions.map(item=>String(item.symbol||''))),unfundedSymbols=new Set(funding.unfunded.map(item=>String(item.symbol||'')));
  ...
  if(!Object.keys(rec).length)return '<div class="decision-empty"><strong>结果记录缺少推荐详情</strong><span>当前记录只有状态信息，请重新读取或等待后台更新。</span></div>';
...
function quantRelativeCandidatesHtml(quant){
  const rec=quant.recommendation||{},all=rec.relative_recommendations||[],affordable=rec.affordable_relative_recommendations||[],capital=Number(quant.request?.capital||0);
  const cards=rows=>rows.map(item=>`<article class="quant-plain-stock watch"><header><div><strong>综合第 ${item.relative_rank} 名 · ${safe(item.name)}</strong><small>${safe(item.symbol)}</small></div><span>${item.screening_passed?'当前宽松度筛选通过':'相对领先 · 优先研究'}</span></header><p>综合评分 ${Number(item.composite_score).toFixed(3)} · 模型上涨概率 ${item.probability_up==null?'暂无':(Number(item.probability_up)*100).toFixed(1)+'%'} · 近期涨跌 ${item.momentum==null?'暂无':(Number(item.momentum)*100).toFixed(1)+'%'}</p><p>需要关注的不足：${safe((item.blockers||[]).map(plainQuantReason).join('；')||'单股筛选通过，但组合验证或资金分配尚未完成')}</p><p>${item.minimum_lot_cost==null?'参考价格缺失，暂不能估算买入金额':`按当前 ${item.minimum_lot_shares} 股单位，含预设佣金与滑点约需 ¥${money(item.minimum_lot_cost,2)} · ${item.capital_affordable?'本金可覆盖':'超出本金'}`}</p><footer><a href="/?view=overview&asset=${encodeURIComponent(item.symbol)}">查看行情与公司资料 →</a></footer></article>`).join('');
  return `<section class="panel relative-candidates"><div class="panel-head"><div><p class="eyebrow">相对优选</p><h2>先看买得起的，再比较谁更值得研究</h2></div><span class="status-tag">已比较 ${rec.relative_candidate_count||0} 只 · 宽松度 ${Number(rec.screening_relaxation||0)} · 筛选通过 ${rec.screening_passed_count||0} 只</span></div><p>对所选板块中历史数据齐备的候选评分，不再先截取成交额前30只。优先展示本金可覆盖的股票；走势和模型概率不足保留为说明，不否决研究排名。</p><div class="subsection-head"><span>本金 ¥${money(capital,0)} 可覆盖的候选</span><small>先看筛选通过，再按综合分排序</small></div>${affordable.length?cards(affordable.slice(0,3))+(affordable.length>3?`<details><summary>查看其余 ${affordable.length-3} 只预算内候选</summary>${cards(affordable.slice(3))}</details>`:''):'<p>本次有评分的候选中，没有本金可覆盖的最小买入单位；上方研究排名仍可查看。</p>'}<details><summary>查看不受本金限制的综合排名</summary><div class="subsection-head"><span>综合排名靠前</span><small>未通过的条件逐只列明</small></div>${cards(all.slice(0,3))}${all.length>3?`<details><summary>查看其余 ${all.length-3} 只相对领先候选</summary>${cards(all.slice(3))}</details>`:''}</details><p class="lab-warning">这里的金额仅用于判断买入单位是否可负担，不是仓位分配；多只金额不能直接相加当作组合。本榜单不受试探仓位 20% 上限筛掉，也不要求平均分配。实际交易单位、价格和费用仍需核验。</p></section>`;
}
...
function quantDecisionPanelHtml(_detail,learning,_quantVersions,sectorCache){
  ...
  return `<section class="panel decision-entry"><div class="panel-head"><div><p class="eyebrow">投资条件</p><h2>填写条件，查看股票推荐</h2></div><div class="daily-decision-state"><span>${dataState}</span><strong>${safe(freshness)}</strong><small>${version?`结果编号 ${safe(version.version_key)}`:'还没有可用结果'}</small></div></div>
    ...
    <div class="decision-form">
      ...
      <label class="decision-sector-field">筛选宽松度 <output id="screeningRelaxationValue">${Number(d.screening_relaxation??50)}</output> / 100<input data-quant-draft="screening_relaxation" id="quickRelaxation" type="range" min="0" max="100" step="10" value="${Number(d.screening_relaxation??50)}"><small id="screeningRelaxationHint">${screeningRelaxationHint(d.screening_relaxation??50)}</small><small>0 保留原门槛 · 50 适度放宽 · 100 最大放宽。调整后点击“按这些条件查看推荐”。</small></label>
    ...
    <section id="quantDecisionOutput" class="panel decision-output" tabindex="-1" aria-label="本次推荐结论"><div class="panel-head"><div><p class="eyebrow">推荐结果</p><h2>本次结论</h2></div><span class="count">研究用途 · 不下单</span></div>${quantDecisionOutputHtml(decision)}</section>`;
}
...
function renderHarness(data,detail){
  ...
  const workflows=(runtime.workflows||[]).filter(item=>item.key!=='strategy_activation').map(item=>`<option value="${safe(item.key)}" ${item.key===state.harnessWorkflow?'selected':''}>${safe(item.label)}</option>`).join('');
  ...
  const experimentOptions=experiments.filter(item=>item.status==='SUCCESS'&&item.activation_eligible).map(item=>`<option value="${safe(item.experiment_key)}">${safe(item.name)} · 自动激活 · ${safe(localTimestamp(item.finished_at))}</option>`).join('')||'<option value="">暂无通过全部门禁的实验</option>';
  const experimentRows=experiments.map(item=>`<div class="harness-row"><div><span class="badge">${safe(item.status)} · ${item.iteration_count||0} 次候选测试</span><strong>${safe(item.name)} · ${safe(item.experiment_key)}</strong><p>${(item.universe||[]).map(stock=>safe(stock.name||stock.symbol)).join('、')}</p><small>数据截至 ${safe(item.data_snapshot_at||'—')} · ${item.activation_eligible?'全部门禁已通过':'门禁未通过，将继续优化'}</small></div><div class="harness-row-actions"><span class="status-tag">${item.activation_eligible?'自动激活':(item.status==='SUCCESS'?'等待新交易日再试':safe(harnessStatus(item.status)))}</span></div></div>`).join('');
...
function screeningRelaxationHint(value){
  const ratio=Number(value)/100;
  return `概率门槛降低 ${(15*ratio).toFixed(1)} 个百分点；均线容忍偏弱 ${(10*ratio).toFixed(0)}%；动量允许跌幅 ${(20*ratio).toFixed(0)}%；财务分门槛降低 ${(0.30*ratio).toFixed(2)}。本金、回撤和资料完整度不变。`;
}
function bindQuantDecisionInputs(){
  document.querySelectorAll('[data-quant-draft]').forEach(element=>{const save=()=>{state.quantDraft[element.dataset.quantDraft]=['number','range'].includes(element.type)?Number(element.value):element.value;if(element.id==='quickRelaxation'){$('#screeningRelaxationValue').textContent=element.value;$('#screeningRelaxationHint').textContent=screeningRelaxationHint(element.value);}persistQuantDraft();markQuantInputChanged();};element.oninput=save;element.onchange=save;});
...
function bindNavigation() {
  const titles = {agent:'投资研究助手',signals:'信号中心',compare:'股票对比',overview:'市场总览',library:'资料搜索',logic:'推荐怎么算',harness:'股票推荐'};
  document.querySelectorAll('.nav-item').forEach(button => button.addEventListener('click', () => {
    ...
    if(button.dataset.view==='agent' && window.loadResearchAgent)window.loadResearchAgent();
  }));
...
async function initialize() {
  try {
    ...
    // Pick up the non-blocking startup/selected-stock fetch without waiting a minute.
    window.setTimeout(refreshRealtimeOverview, 3_000);
  } catch (error) {
...
window.setInterval(refreshMarketSyncStatus, 5_000);
window.setInterval(()=>{if(!document.hidden&&!state.syncStatusError&&state.dashboard?.meta)renderMarketSyncState(state.dashboard.meta);},1_000);
